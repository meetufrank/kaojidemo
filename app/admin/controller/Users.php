...
class Users extends Common{
    ...
    /***********************************会员考点***********************************/
    public function userKaod(){
        if(request()->isPost()){
//            $userLevel=db('user_kaod');
//            $list=$userLevel->order('sort')->select();
           $list=KaodLogic::getInstance()->getKaod();
            return $result = ['code'=>0,'msg'=>'获取成功!','data'=>$list,'rel'=>1];
        }
         
        return $this->fetch();
    }
    public function KaodAdd(){
        if(request()->isPost()){
            $data = input('post.');
            $data['open'] = input('post.open') ? input('post.open') : 0;
            db('user_kaod')->insert($data);
            $result['msg'] = '会员考点添加成功!';
            $result['url'] = url('userKaod');
            $result['code'] = 1;
            return $result;
        }else{
            $this->assign('title',lang('add')."会员考点");
            $this->assign('info','null');
            return $this->fetch('KaodForm');
        }
    }
    public function KaodEdit(){
        if(request()->isPost()) {
            $data = input('post.');
            db('user_kaod')->update($data);
            $result['msg'] = '会员考点修改成功!';
            $result['url'] = url('userKaod');
            $result['code'] = 1;
            return $result;
        }else{
            $map['kd_id'] = input('param.kd_id');
            $info = db('user_kaod')->where($map)->find();
            $this->assign('title',lang('edit')."会员考点");
            $this->assign('info',json_encode($info,true));
            return $this->fetch('KaodForm');
        }
    }
    public function KaodDel(){
        $kd_id=input('kd_id');
        if (empty($kd_id)){
            return ['code'=>0,'msg'=>'考点ID不存在！'];
        }
        db('user_kaod')->where(array('kd_id'=>$kd_id))->delete();
        return ['code'=>1,'msg'=>'删除成功！'];
    }
    public function KaodOrder(){
        $userLevel=db('user_kaod');
        $data = input('post.');
        $userLevel->update($data);
        $result['msg'] = '排序更新成功!';
