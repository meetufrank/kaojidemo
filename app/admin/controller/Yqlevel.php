...
class Yqlevel extends Common{
    ...
    public function index(){
        if(request()->isPost()){
            ...
            $order = "y.listorder asc,y.id desc";
            if (input('post.catid')) {
            ...
            if(!empty($keyword) ){
                $map['y.title|t.title']=array('like','%'.$keyword.'%');
            }
            ...
            $list = $model
                ->alias('y')
                ->join(config('database.prefix').'type t','y.yqid = t.id','left')
                ->where($map)
                ->order($order)
                ->field('y.*,t.title as yqtitle,t.title_style as yqtitle_style,t.thumb as yqthumb')
            ...
            return $this->fetch ('yqlevel/index');
        }
    }
//设置乐器状态
    public function usersState(){
        ...
        if(db('yqlevel')->where('id='.$id)->update(['is_open'=>$is_lock])!==false){
    ...
    public function edit(){
        ...
        return $this->fetch('yqlevel/edit');
    }
    function update(){
        $request = Request::instance();
